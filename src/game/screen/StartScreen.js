...
export default class StartScreen extends Screen{
	...
	build(){
		...
		//create start button
		this.circleLock = new PIXI.Container();
		this.innercircleLockShape = this.drawCircle(0xffffff,this.puzzleRadius * 0.9);
		this.circleLock.addChild(this.innercircleLockShape);
		this.addChild(this.circleLock);
		this.lockImage = new PIXI.Sprite(PIXI.Texture.fromImage('./assets/Asset-2.png'));
		this.circleLock.addChild(this.lockImage);
		this.lockImage.tint = 0xe05d26;
		this.lockImage.x -= this.lockImage.width / 2;
		this.lockImage.y -= this.lockImage.height / 2;

		this.circleLock.x = config.middlePoint.x;
		this.circleLock.y = config.middlePoint.y;
		...
		this.hard.scale = {x:0.75,y:0.75};
		this.games.scale = {x:0.75,y:0.75};

		this.hard.x = config.width / 2 - this.hard.width - 80;
		this.hard.y = config.height / 2 - this.hard.height / 2;


		this.games.x = config.width / 2 + 80;
		...
		this.startButton.on('tap', this.toGame.bind(this)).on('click', this.toGame.bind(this));
		...
		this.animatedBuzzleBorder.lineStyle(this.puzzleBorderSize, 0xeaeaea);
	...
	toGame(){
		TweenLite.killTweensOf(this.animatedBuzzleBorder);
		this.endState = true;
	...
	transitionOut(nextScreen){
		...
		this.foregroundColor = 0x007d7c;
		this.foreground.endFill();
		this.foreground.tint = this.foregroundColor;
		this.foreground.alpha = 0;

		// TweenLite.from(this.foreground,0.5,{ y:-this.foreground.height});
		// TweenLite.to(this.mainContainer,0.25,{y:config.height/2 - 20});
		// TweenLite.to(this.mainContainer,0.5,{y:config.height*2, delay:0.2});
		// TweenLite.to(this.mainContainer.scale,0.2,{x:0.8, y:1.2});
		// TweenLite.to(this.mainContainer.scale,0.5,{x:1.2, y:0.8,delay:0.2, onComplete:this.endTransitionOut.bind(this)});


		// TweenLite.to(this.startButton,0.3,{x:config.width + 140, ease:'easeInBack'});

		TweenMax.to(this.startButton, 1.2, {
			bezier: {
				type:"thru",
				curviness :2,
				values:[
					{x:this.startButton.x, y:this.startButton.y}, 
				  	{x:config.middlePoint.x + (this.startButton.x - config.middlePoint.x) / 2, y:config.height - this.puzzleRadius},
				  	{x:config.middlePoint.x, y:0}],
				  	ease:Power1.easeInOut
				}, onComplete:this.endTransitionOut.bind(this)				
			}
		);
		this.endState = true;
		this.animatedBuzzleBorder.alpha = 0;
		TweenLite.to(this.startButton.scale,1,{x:20, y:20,ease:'easeInCubic'});
		TweenLite.to(this.arrowImage,0.2,{alpha:0});

		// TweenLite.to(this.startButton,0.3,{x:config.width + 140, ease:'easeInBack', onComplete:this.endTransitionOut.bind(this)});
	}
